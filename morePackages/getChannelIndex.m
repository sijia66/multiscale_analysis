...
if ~isempty(Electrode)
    if isfield(Trials,'ChannelID')
        for iElectrode = 1:length(Trials(1).ChannelID(Microdrive).Electrode)
            ChannelIDs(iElectrode) = Trials(1).ChannelID(Microdrive).Electrode(iElectrode);
        end
        ChannelIndex = zeros(1,length(Electrode));
        for iElectrode = 1:length(Electrode)
            ChannelIndex(iElectrode) = find(ChannelIDs==Electrode(iElectrode));
        end
    elseif isfield(Trials,'NumContacts')
