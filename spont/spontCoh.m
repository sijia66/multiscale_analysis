...
%Modifications
%
%Jan 24, 2019  Taken out the parallel processing part
%
...
defaults   = {'lfp',      2,        5,                30,      2}; %#ok<NASGU>
...
specDat(nDrive) = struct('PS', [], 'freq', [], 'drive', [], 'datType', []);
cohDat(sum(1:nDrive)) = struct('coh', [], 'freq', [], 'driveIDs', [], 'category', []);
cnt = 1;
...
for iD1=1:nDrive
    ...
    elseif isempty(data(iD1).tsDat_tr)
        disp('Creating arbitrary trials...')
        data(iD1).tsDat_tr = spontMakeTrials(data(iD1).tsDat, datParams.trLen, Fs);
    end
    ...
    elseif datParams.sigmaThreshold > 0 & isempty(data(iD1).artTr)
        disp('Finding outlier trials...')
        data(iD1).artTr = spontFindArtTrials(data(iD1).tsDat_tr, datParams.sigmaThreshold);
    end
...
%         end
        ...
        coh_tmp2 = nan(nE1, nE2, nF);
        spec_tmp2 = nan(nE1, nF);
